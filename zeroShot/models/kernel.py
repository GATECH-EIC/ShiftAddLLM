...
def unpack_weight(bW:torch.Tensor, alpha:torch.Tensor):
    ...
    ret_weight = bW.new_zeros(K, N, dtype=torch.half)
...
def load_shiftaddllm_weight(model, weights_dir, model_name, wbits, groupsize=-1, quant_type="blockwise"):
    ...
    for i in tqdm(range(len(layers)), desc="Loading shiftaddllm low-bit weights", leave=False):
        ...
        for name in subset:
            layer_name = f"{i}.{name}"
            temp_storage_pt = os.path.join(weights_dir, f"{model_name}_{layer_name}_{wbits}bit_{groupsize}groupsize_{quant_type}.pt")
